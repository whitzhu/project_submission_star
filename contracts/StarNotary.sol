...
contract StarNotary is ERC721 {
    ...
    function exchangeStars(uint256 _tokenId1, uint256 _tokenId2) public {
        require(
            ...
            "Owner must own either one of the token that's being exchanged"
        );
        bool isSenderOwnerOfToken1 = ownerOf(_tokenId1) == msg.sender;
        address ownerOfToken1 = ownerOf(_tokenId1);
        address ownerOfToken2 = ownerOf(_tokenId2);
        if (isSenderOwnerOfToken1) {
            transferFrom(ownerOfToken1, ownerOfToken2, _tokenId1);
            transferFrom(ownerOfToken2, ownerOfToken1, _tokenId2);
        } else {
            transferFrom(ownerOfToken2, ownerOfToken1, _tokenId1);
            transferFrom(ownerOfToken1, ownerOfToken2, _tokenId2);
        }
    ...
    function transferStar(address _to1, uint256 _tokenId) public {
        require(ownerOf(_tokenId) == msg.sender, "Sender must own token");
